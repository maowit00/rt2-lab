...
int32 e_rpm;                      // rpm control: control error 
int32 rpm_control_u;              // rpm control: control output 

const int32 max_rpm_control_output = +127;
...
// Position control               
int32 set_position;               // set position in ticks (400 ticks per rotation)
int32 e_position;                 // position control: control error 
const int32 max_pos_control_output = +511; 
const int32 min_pos_control_output = -512;
...
// *******************************************************************************************
// int32 set_position;                   // Sollwert in Ticks (400 Ticks pro Umdrehung)
// int32 pos_tick;                       // Istposition in Ticks
// *********************************************************************************
// Groessen, die mit einem sinnvollen Wert belegt werden muessen: 
// *********************************************************************************
// const int32 kp_position               // Reglerparameter: Proportionalbeiwert
// *******************************************************
// Groessen, die berechnet werden muessen; sind schon deklariert
// *******************************************************
// int32 e_position;                     // Regelfehler
// int32 set_rpm;                        // Stellgroesse des Positionsreglers bzw. Sollwert fuer den unterlagerten Drehzahlregler
//
void positionControl(void)
{
  int32 kp_position_N = 0.3 * (1<<POS_N); // 38

  e_position  = set_position - pos_tick;          // position control: control error

  set_rpm = (kp_position_N * e_position) >> POS_N1;
}
...
//#############################################################################################             
// *******************************************************************************************
// Groessen, die gelesen werden koennen. Diese Groessen sind an dieser Stelle bereits mit Werten belegt:
// *******************************************************************************************
// int32 set_rpm;                        // Drehzahl--Sollwert: passend skaliert zu measured_rpm
// int16 measured_rpm;                   // Gemessener Drehzahlwert vom ADC -512 .. + 511 --> -7800 ... +7800 U/min
// RtiPeriodNS                           // Abtastzeit = RTI Zykluszeit in ns
// const int32 max_rpm_control_output    // Stellgroessenbeschraenkung oben
// const int32 min_rpm_control_output    // Stellgroessenbeschraenkung unten
// *********************************************************************************
// Groessen, die mit einem sinnvollen Wert belegt werden muessen: 
// *********************************************************************************
// const int32 kp_rpm                    // Reglerparameter: Proportionalbeiwert
// const int32 Tn_rpm                    // Reglerparameter: Nachstellzeit
// *******************************************************
// Groessen, die berechnet werden muessen; sind schon deklariert
// *******************************************************
// int32 e_rpm;                          // Regelfehler
// int32 rpm_control_u;                  // Stellgroesse des Drehzahlreglers
// Sie muessen gegebenenfalls weitere Variablen lokal deklarieren
//
void rpmControl(void)
{
   const int32 kp_rpm_N = 1.2 * (1<<RPM_NN);                //153
   const int32 ki_rpm_M = 1.2 * 1536 / 51100 * (1<<RPM_NI); //590
   int32 local_measured_rpm;

   int32 up_rpm;
   int32 ui_rpm_M;               // neuer Wert des I-Anteils
   static int32 ui_rpm_M_1 = 0;  // alter Wert des I-Anteils

   // scale input signals depending on postion control or not and limit set value for speed
   if (control_state == position){
       local_measured_rpm = measured_rpm << POS_N2;
       set_rpm = (set_rpm > (max_pos_control_output<<POS_N2)) ? (max_pos_control_output<<POS_N2) : set_rpm;
       set_rpm = (set_rpm < (min_pos_control_output<<POS_N2)) ? (min_pos_control_output<<POS_N2) : set_rpm;             
   }else{
       local_measured_rpm = measured_rpm;
   ...
   e_rpm = set_rpm - local_measured_rpm;
   // Sättigung Regelfehler
   e_rpm = (e_rpm >  1000000) ?  1000000 : e_rpm;
   e_rpm = (e_rpm < -1000000) ? -1000000 : e_rpm;

   up_rpm   = (kp_rpm_N * e_rpm) >> RPM_NN;                 // P Anteil
   ui_rpm_M = sat_add_int32( ki_rpm_M * e_rpm, ui_rpm_M_1); // I Anteil (skaliert)
   rpm_control_u = sat_add_int32( up_rpm, ui_rpm_M >> RPM_NI);    // Stellgröße

   if (control_state == position){
      rpm_control_u >>= POS_N2; 
   }

   // Anti-Windup und Sättigung der Stellgröße
...
{ uint16 t0, t0H, t1, t1H;
  ...
  if(    measured_rpm >  (((int32)MAX_ABS_RPM_POS_SLOW)<<CONV_ADC_N_SHIFT)/CONV_ADC_RPM_UPMIN 
      || measured_rpm < -(((int32)MAX_ABS_RPM_POS_SLOW)<<CONV_ADC_N_SHIFT)/CONV_ADC_RPM_UPMIN ){
...
{   uint8 pwm_out;
    ...
    if( rti_adc_started ){
         ...
         if(    pac_position_decoding 
             && measured_rpm <  (((int32)MIN_ABS_RPM_POS_PAC)<<CONV_ADC_N_SHIFT)/CONV_ADC_RPM_UPMIN 
             && measured_rpm > -(((int32)MIN_ABS_RPM_POS_PAC)<<CONV_ADC_N_SHIFT)/CONV_ADC_RPM_UPMIN ){
...
//*********************************************************************************************
         ...
         if( control_state == rpm && display_state == set_val_gen ){  // Automatic stimulus for rpm set value (rpm control mode)
              set_rpm = (wav_gen_ticks < WAV_GEN_HIGH_COUNT) ? 
                         (((int32)WAV_GEN_HIGH_LEVEL_RPM)<<CONV_ADC_N_SHIFT)/CONV_ADC_RPM_UPMIN
                       : (((int32)WAV_GEN_LOW_LEVEL_RPM )<<CONV_ADC_N_SHIFT)/CONV_ADC_RPM_UPMIN;
...
//*********************************************************************************************
         ...
         // Check for overload
         if( overload_I2dt((((int32)measured_current)*CONV_ADC_I_mA)>>CONV_ADC_I_SHIFT) || error_msg == err_txt_I2dt_overload ){
        ...
        LedSet(0x10);
        if(control_state == position) {   
            softScope((int16) pos_tick, (int16)((int32)measured_rpm*CONV_ADC_RPM_UPMIN/(1<<CONV_ADC_N_SHIFT)));   
        } else if(control_state == rpm) {   
            softScope((int16) ((int32)set_rpm*CONV_ADC_RPM_UPMIN/(1<<CONV_ADC_N_SHIFT)),  (int16)((int32) measured_rpm*CONV_ADC_RPM_UPMIN/(1<<CONV_ADC_N_SHIFT)));  

        } else if(control_state == manual)
        {   
//         softScope((int16) measured_rpm_adc, (int16) measured_rpm_ttl);  
           softScope((int16)((int32) measured_rpm_adc*CONV_ADC_RPM_UPMIN/(1<<CONV_ADC_N_SHIFT)), (int16)((int32) measured_rpm_ttl*CONV_ADC_RPM_UPMIN/(1<<CONV_ADC_N_SHIFT)));  
        }
...
{   
    ...
    for(;;){
       ...
       switch( control_state ){
            case rpm      : (void) sprintf(lcd_line_0, "R:w=%4d e=%4d", 
                                        (int16)(set_rpm*CONV_ADC_RPM_UPMIN/(1<<CONV_ADC_N_SHIFT)),       
                                        (int16)((int32)e_rpm*CONV_ADC_RPM_UPMIN/(1<<CONV_ADC_N_SHIFT))); 
                            switch( display_state ){
                               case disp_only : (void) sprintf(lcd_line_1, "y=%4d u=%4d", 
                                        (int16)((int32)measured_rpm*CONV_ADC_RPM_UPMIN/(1<<CONV_ADC_N_SHIFT)), 
                                        (int16)(rpm_control_u));    
                            }
                            break;
            case position : (void) sprintf(lcd_line_0, "P:w=%2.2f e=%4d", 
                                        (((float)set_position)/CONV_ROTATIONS_POS), 
                                        (int16)e_position);    
                            switch( display_state ){                                                    
                               case disp_only : (void) sprintf(lcd_line_1, "y=%2.2f u=%4d", 
                                        (((float)pos_tick)/CONV_ROTATIONS_POS), 
                                        (int16)((int32)(set_rpm)*CONV_ADC_RPM_UPMIN/(1<<CONV_ADC_N_SHIFT)));    
                            }
                            break;
            case manual   : (void) sprintf(lcd_line_0, "M:u=%3.0ld n=%5d", manual_u, (int16)((int32)measured_rpm*CONV_ADC_RPM_UPMIN/(1<<CONV_ADC_N_SHIFT))); 
                            switch( display_state ){                                                    
       ...
       switch( display_state ){
             ...
             case set_val_gen     : (void) sprintf(lcd_line_1, "AutoStim %3.0fus", measuredTimeInUsec);
                                    break;
             case set_val_inc_dec : (void) sprintf(lcd_line_1, "SW4:- 5:+ %5d", (int16)((int32)measured_rpm_filtered * CONV_ADC_RPM_UPMIN/(1<<CONV_ADC_N_SHIFT)));
